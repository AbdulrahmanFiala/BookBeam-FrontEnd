...
export const PaymentPage = () => {
  ...
  useEffect(() => {
    const fetchFees = async () => {
      if (authState && authState.isAuthenticated) {
        const url = `${process.env.REACT_APP_BACKEND_BASE_URL}/payments/search/findByUserEmail?userEmail=${authState.accessToken?.claims.sub}`;
  ...
  async function checkout() {
    ...
    const url = `${process.env.REACT_APP_BACKEND_BASE_URL}/payment/secure/payment-intent`;
    const requestOptions = {
      method: "POST",
      headers: {
    ...
    stripe
      ...
      .then(async function (result: any) {
        if (result.error) {
          ...
          const url = `${process.env.REACT_APP_BACKEND_BASE_URL}/payment/secure/payment-complete`;
          const requestOptions = {
            method: "PUT",
            headers: {
              Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
